...
def beta1_beta2(portal):
    ...
    if not propSheet.hasProperty('large_site'):
        ...
        logger.info("Added 'large_site' property to site_properties.")
    ...
    simplifyActions(portal)

    # Use the @@plone view for the RTL.css expression entry
    migrateCSSRegExpression(portal)


def addDragDropReorderJS(portal):
    """Add dragdropreorder.js to ResourceRegistries.
    """
...
def addGetEventTypeIndex(portal):
    ...
    if catalog is not None:
        if 'getEventType' in catalog.indexes():
            ...
            logger.info("Deleted %s 'getEventType' from portal_catalog." %
                       indextype)

        catalog.addIndex('getEventType', 'KeywordIndex')
        logger.info("Added KeywordIndex 'getEventType' to portal_catalog.")
        return 1 # Ask for reindexing
    return 0


def fixHomeAction(portal):
    ...
    if actionsTool is not None:
        ...
        logger.info("Added/modified home/index_html portal_tabs action.")


def removeBogusSkin(portal):
    ...
    if skins is not None:
        if skins._getSelections().has_key('cmf_legacy'):
            ...
            logger.info("Deleted incorrectly added 'cmf_legacy' skin")

def addPloneSkinLayers(portal):
    st = getToolByName(portal, 'portal_skins', None)
    if st is None:
        logger.info('No portal_skins tool')
        return
    ...
    for skin in st._getSelections().keys():
        ...
        if not 'plone_deprecated' in path:
            ...
            logger.info('Added plone_deprecated to %s' % skin)
...
def migrateCSSRegExpression(portal):
        ...
        if aq_base(resource) is not None:
            ...
            if "object.isRightToLeft" in css_expr or \
               "object.restrictedTraverse('@@plone')" in css_expr:
                ...
                logger.info("Fixed RTL.css expression to use the @@plone view")
