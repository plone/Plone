...
class TestContentPublishing(PloneTestCase.PloneTestCase):
    ...
    def testIsExpiredWithImplicitNotExpiredContentByContributor(self):
        self.setRoles(['Contributor'])
        self.folder.invokeFactory('Document', id = 'd1', title = 'Doc 1')
        self.folder.d1.content_status_modify(workflow_action = 'submit')
        msgs = self.portal.plone_utils.showPortalMessages()
        self.assertEqual(msgs[0].message, u'Item state changed.')
        self.assertEqual(msgs[0].type, u'info')
        self.folder.d1.content_status_modify(workflow_action = 'pending',
                                             expiration_date = '1/2/2001')
        self.failIf(self.folder.d1.isExpired())
        msgs = self.portal.plone_utils.showPortalMessages()
        self.assertEqual(msgs[0].message, u'Item state not changed due to invalid permission.')
