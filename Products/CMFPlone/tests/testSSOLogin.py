...
class TestSSOLogin(ptc.FunctionalTestCase):
    ...
    def test_loginAndLogout(self):
        ...
        header = browser.headers['Set-Cookie']
        assert '__ac' in header
        assert self.login_portal.absolute_url_path() in header
        # Without javascript we must click through
        browser.getForm('external_login_form').submit()
        header = browser.headers['Set-Cookie']
        assert '__ac' in header
        assert self.portal.absolute_url_path() in header
        # Now logout
        browser.open(self.portal.absolute_url())
